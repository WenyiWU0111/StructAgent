...
from lib_misc import draw_coordinates
from lib_misc import set_current_result_dir
...
def run_single_example_os_symphony(agent, env, example, max_steps, instruction, args, example_result_dir, scores):
    ...
    # [ADAPTATION W-C] text-answer benchmarks (mind2web / mmina_*): swap the
    # answer sentinel in the task hint to Symphony's own channel
    # (agent.done(return_value=...) -> os_aci.returned_info), and clear any
    # answer left from the previous episode (agents are reused per worker).
    from lib_run_single_text_answer import (
        rewrite_text_answer_instruction, text_answer_eval_mode)
    text_eval_mode = text_answer_eval_mode(example)
    if text_eval_mode:
        instruction = rewrite_text_answer_instruction(
            instruction, "agent.done(return_value=")
        _aci = getattr(agent, "os_aci", None)
        if _aci is not None:
            _aci.returned_info = None
    ...
    end_time = time.time()
    if text_eval_mode:
        # [ADAPTATION W-C] text-answer benchmarks skip env.evaluate(); score
        # via the shared scorer (byte-identical judge/matcher to the main
        # harness rows). Must run while the env lives — the URL
        # force-extract reads the live Chrome tab via CDP.
        from lib_run_single_text_answer import (
            collect_step_screenshots_b64, score_text_answer)
        raw_answer = getattr(
            getattr(agent, "os_aci", None), "returned_info", None)
        if isinstance(raw_answer, str):
            agent_answer = raw_answer
        elif raw_answer is not None:
            agent_answer = json.dumps(raw_answer, ensure_ascii=False)
        else:
            agent_answer = ""
        example.setdefault("_eval_mode", text_eval_mode)
        example.setdefault(
            "_text_answer_domain",
            example.get("_mmina_subdomain") or "mind2web")
        result = float(score_text_answer(
            example=example,
            instruction=instruction,
            agent_answer=agent_answer,
            screenshots_b64=collect_step_screenshots_b64(example_result_dir),
            env=env,
