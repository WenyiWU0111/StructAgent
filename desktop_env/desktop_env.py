...
class DesktopEnv(gym.Env):
    ...
    def reset(self, task_config: Optional[Dict[str, Any]] = None, seed=None, options=None) -> Dict[str, Any]:
        ...
        for attempt in range(MAX_RETRIES):
            ...
            if task_config is not None:
                ...
                if success:
                    ...
                    # If setup cannot reach the VM service, force a restart path
                    # on the next loop instead of repeatedly probing a dead endpoint.
                    self.is_environment_used = True
                    time.sleep(5)
            else:
                setup_success = True
        ...
        if not setup_success:
            raise RuntimeError(f"Environment setup failed after {MAX_RETRIES} retries.")
