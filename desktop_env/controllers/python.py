...
class PythonController:
    ...
    def run_bash_script(self, script: str, timeout: int = 30, working_dir: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        """
        expanded_working_dir = os.path.expanduser(working_dir) if working_dir else None
        bash_script = script
        if expanded_working_dir:
            quoted_dir = shlex.quote(expanded_working_dir)
            bash_script = f"cd {quoted_dir} && {script}"
        payload = json.dumps({
            "command": ["/bin/bash", "-lc", bash_script],
            "shell": False
        })
        ...
        for _ in range(self.retry_times):
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.info("Bash script executed via /execute with return code: %d", returncode)
                    return result
                else:
